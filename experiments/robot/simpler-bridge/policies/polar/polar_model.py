...
try:
    ...
    from transformers import AutoConfig, AutoImageProcessor, AutoModelForImageTextToText as AutoModelForVision2Seq, AutoProcessor
...
def get_vla(pretrained_checkpoint: str):
    ...
    # Load VLA checkpoint.
    attn_implementation = os.environ.get("POLAR_EVAL_ATTN_IMPLEMENTATION", "flash_attention_2")
...
_TRANSFORMER_UTILS_PATH = Path(__file__).resolve().parents[5] / "prismatic" / "models" / "policy" / "transformer_utils.py"
_TRANSFORMER_UTILS_SPEC = importlib.util.spec_from_file_location("polar_transformer_utils", _TRANSFORMER_UTILS_PATH)
_TRANSFORMER_UTILS = importlib.util.module_from_spec(_TRANSFORMER_UTILS_SPEC)
assert _TRANSFORMER_UTILS_SPEC.loader is not None
_TRANSFORMER_UTILS_SPEC.loader.exec_module(_TRANSFORMER_UTILS)
MAPBlock = _TRANSFORMER_UTILS.MAPBlock


class ActionDecoderHead(torch.nn.Module):
    def __init__(self, window_size: int = 5, hidden_dim: int = 512, latent_action_token_len: int = 5):
        super().__init__()
...
class ActionDecoder(nn.Module):
    def __init__(self, window_size=10, latent_action_token_len: int = 5):
        super().__init__()
        self.net = ActionDecoderHead(window_size=window_size, latent_action_token_len=latent_action_token_len)
...
class PoLARBridgeInference:
    def __init__(
        self,
        saved_model_path: str = "polar_bridge_vla",
        decoder_path: str = "polar_bridge_vla/action_decoder.pt",
        ...
        self._explicit_unnorm_key = unnorm_key is not None
    ...
    def _resolve_unnorm_key(self, task_description: str) -> None:
        if self._explicit_unnorm_key:
            return
        ...
        for key in norm_stats.keys():
            if str(key).lower() in task:
                if self.unnorm_key != key:
                    print(f"[*] Using action normalization stats: {key}")
                self.unnorm_key = key
                return
        if len(norm_stats) == 1:
            ...
            return
        raise ValueError(
            "Could not infer action normalization key from task "
            f"{task_description!r}; available keys: {list(norm_stats.keys())}"
        )

    def reset(self, task_description: str) -> None:
        if isinstance(task_description, (list, tuple)):
            task_description = task_description[0] if task_description else ""
        self.task_description = task_description
        self._resolve_unnorm_key(task_description)
        self.num_image_history = 0
