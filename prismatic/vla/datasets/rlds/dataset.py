...
def make_dataset_from_rlds(
    ...
    *,
    ...
    split_override = os.environ.get("POLAR_RLDS_TRAIN_SPLIT" if train else "POLAR_RLDS_VAL_SPLIT")
    if split_override:
...
def apply_trajectory_transforms(
    ...
    *,
    ...
    if training_phase == 'post-training':
        transform = traj_transforms.chunk_act_obs_window     # load all obs. within a window
    else:       
...
def make_single_dataset(
    ...
    *,
    ...
    ram_budget_mb = int(os.environ.get("POLAR_TF_RAM_BUDGET_MB", "1"))
    if ram_budget_mb > 0:
    ...
    *,
    ...
    for dataset_kwargs in dataset_kwargs_list:
        ...
        if "num_transitions" in dataset_statistics:
            dataset_size = dataset_statistics["num_transitions"]
        elif os.environ.get("POLAR_RLDS_LEN_OVERRIDE"):
            dataset_size = int(os.environ["POLAR_RLDS_LEN_OVERRIDE"])
        else:
            raise KeyError("dataset_statistics is missing 'num_transitions'; set POLAR_RLDS_LEN_OVERRIDE for smoke runs.")
    ...
    for dataset_kwargs, threads, reads in zip(
        ...
        dataset_kwargs_for_load = copy.deepcopy(dataset_kwargs)
        dataset_kwargs_for_load.pop("dataset_statistics", None)
        dataset, _ = make_dataset_from_rlds(
            **dataset_kwargs_for_load,
            train=train,
